...
Additionally, after downloading developer version of IBM MQ 9 (`mqadv_dev914_linux_x86-64.tar.gz`), we
can find additional library package:

`MQSeriesJava-9.1.4-0.x86_64.rpm` -> `MQSeriesJava-9.1.4-0.x86_64.cpio`.
After unpacking cpio package, we can see even more complete set of libraries.

Even if it seems that `9.1.5.0-IBM-MQ-Install-Java-All` package should be used there are some OSGi issues
...
* `OSGi` contains respective `com.ibm.mq.osgi.allclient_9.1.5.0.jar` and dependencies

`MQSeriesJava-9.1.4-0.x86_64.cpio` contains (`/opt/mqm/java/lib/OSGi`) these OSGi packages:

* `com.ibm.mq.osgi.allclientprereqs_9.1.4.0.jar` - should not be used
* `com.ibm.mq.osgi.allclient_9.1.4.0.jar` - depends on the above, so should not be used
* `com.ibm.mq.osgi.java_9.1.4.0.jar`
* `com.ibm.msg.client.osgi.commonservices.j2se_9.1.4.0.jar`
* `com.ibm.msg.client.osgi.jms_9.1.4.0.jar` - should not be used
* `com.ibm.msg.client.osgi.jms.prereq_9.1.4.0.jar`
* `com.ibm.msg.client.osgi.nls_9.1.4.0.jar`
* `com.ibm.msg.client.osgi.wmq_9.1.4.0.jar`
* `com.ibm.msg.client.osgi.wmq.nls_9.1.4.0.jar`
* `com.ibm.msg.client.osgi.wmq.prereq_9.1.4.0.jar`

This quickstarts manual is not a definitive guide about which IBM MQ access methods are the canonical ones.

== A-MQ 7 installation (Artemis)

This time we'll run standalone (no docker) version of `amq-broker-7.6.0-bin.zip`.

[listing,options="nowrap"]
----
$ pwd
/data/servers/amq-broker-7.6.0

$ bin/artemis create --user fuse --password fuse --require-login amq7
Creating ActiveMQ Artemis instance at: /data/servers/amq-broker-7.6.0/amq7
...
You can now start the broker by executing:
   ...
   "/data/servers/amq-broker-7.6.0/amq7/bin/artemis" run

Or you can run the broker in the background using:

   "/data/servers/amq-broker-7.6.0/amq7/bin/artemis-service" start
...
2020-05-11 15:40:21,742 INFO  [org.apache.activemq.artemis.integration.bootstrap] AMQ101000: Starting ActiveMQ Artemis Server
...
----
...
* `mvn:org.apache.activemq/artemis-core-client/2.11.0.redhat-00005`
* `mvn:org.apache.activemq/artemis-jms-client/2.11.0.redhat-00005`
